...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        return http.authorizeHttpRequests(configurer -> configurer
                        .requestMatchers("/api/assignee/**").hasRole("ASSIGNEE")
    ...
    /**
     * Создает бин для обработки аутентификации пользователей через OpenID Connect.
     * Этот сервис расширяет стандартные данные пользователя, добавляя роли (если они есть) из claim "groups".
     *
     * @return OAuth2UserService для обработки запросов на аутентификацию пользователей
     */
    @Bean
